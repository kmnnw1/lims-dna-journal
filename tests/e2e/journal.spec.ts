...
async function loginAdmin(page: Page) {
	...
	// Находим поле для токена (в нашем MD3 интерфейсе это input type="password")
	...
	await expect(page.getByRole('heading', { name: /Журнал Проб/i })).toBeVisible({
		timeout: 20000,
	});
}
...
test.describe('Журнал Проб - Основной функционал', () => {
	...
	test('Проверка новых функций (Экспорт, Сканер)', async ({ page }) => {
		...
		await expect(
			page
				.locator('button:visible')
				.filter({ hasText: /Выгрузить CSV/i })
				.first(),
