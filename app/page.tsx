...
export default function JournalPage() {
    ...
    useEffect(() => { setPage(1); }, [debouncedSearch, filterType, sortConfig]);

    const fetchSpecimens = async () => {
        setLoading(true);
        try {
            const params = new URLSearchParams({
                page: page.toString(), limit: '50', search: debouncedSearch, filter: filterType,
                sortBy: sortConfig?.key || 'id', sortOrder: sortConfig?.direction || 'asc'
            });
                ...
                <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 custom-scrollbar">
                    {(['all', 'success', 'error', 'fav'] as const).map((type) => (
                        ...
                        >
                            ...
                            {type === 'fav' && <span className="flex items-center gap-1"><Star className="w-4 h-4" /> Избранное</span>}
                        </button>
                    ))}
                </div>
