...
import { FlaskConical, Beaker, Download, Plus, LogOut, Settings, Trash2, Wifi, WifiOff, Activity, Printer, Moon, Sun, Search, MoreHorizontal, Star, Link2, Keyboard, Pencil, FileSpreadsheet, Filter, RefreshCw, FileText, ScanLine, Home as HomeIcon } from 'lucide-react';
...
const MD3 = {
  ...
  btnSecondary: "inline-flex items-center justify-center gap-2 rounded-full bg-teal-50 px-5 py-3 text-sm font-bold text-teal-900 hover:bg-teal-100 dark:bg-teal-900/30 dark:text-teal-100 dark:hover:bg-teal-900/50 active:scale-95 transition-all",
  ...
  tableRow: "hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors border-b border-zinc-100 dark:border-zinc-800/50 last:border-0",
...
function Home() {
  ...
  useEffect(() => {
    const onKey = (e: globalThis.KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        setIsAddModalOpen(false); setEditingSpecimen(null); setPcrModalId(''); setScanOpen(false); setToolsSheetOpen(false); setPaletteOpen(false); setShortcutsOpen(false);
      }
    };
  ...
  const toggleStatus = async (id: string, current: string, markerKey: string = 'itsStatus') => {
    ...
    const res = await fetch('/api/specimens', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ singleId: id, updateData: { [markerKey]: nextStatus } }) });
    const result = await parseApiResponse(res);
    if (!result.ok) { showToast(result.message); return; }
    fetchSpecimens();
  };
  ...
  const handleAddAttempt = async () => {
    ...
    const res = await fetch('/api/specimens', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ 
      newAttempt: { specimenId: pcrModalId, volume: pcrForm.volume, result: pcrForm.result, marker: pcrForm.marker.trim(), forwardPrimer: pcrForm.forwardPrimer.trim(), reversePrimer: pcrForm.reversePrimer.trim(), dnaMatrix: pcrForm.dnaMatrix.trim() } 
    })});
    const result = await parseApiResponse(res);
    if (!result.ok) { showToast(result.message); return; }
  ...
  const exportCsv = () => {
    try { exportToCsv(filteredSpecimens); } catch { showToast('Не удалось сформировать CSV'); }
  };
  ...
  const renderMarkerStatus = (s: any, marker: 'ITS' | 'SSU' | 'LSU' | 'MCM7') => {
    const key = marker === 'SSU' ? 'ssuStatus' : marker === 'LSU' ? 'lsuStatus' : marker === 'MCM7' ? 'mcm7Status' : 'itsStatus';
    const st = s[key];
    const base = 'inline-flex items-center justify-center rounded-full px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider transition-all cursor-pointer select-none';
    
    if (st === '1') return <button type="button" onClick={() => !isReader && toggleStatus(s.id, st, key)} className={`${base} bg-emerald-100 text-emerald-800 hover:bg-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-300`}>{marker} ✓</button>;
    if (['badQ', 'badDNA', 'bad'].includes(String(st))) return <button type="button" onClick={() => !isReader && toggleStatus(s.id, st, key)} className={`${base} bg-rose-100 text-rose-800 hover:bg-rose-200 dark:bg-rose-900/40 dark:text-rose-300`}>{marker} ✕</button>;
    if (['alien', 'fungus'].includes(String(st))) return <button type="button" onClick={() => !isReader && toggleStatus(s.id, st, key)} className={`${base} bg-amber-100 text-amber-900 hover:bg-amber-200 dark:bg-amber-900/40 dark:text-amber-300`}>{marker} 👽</button>;
    
    return <button type="button" onClick={() => !isReader && toggleStatus(s.id, st || '', key)} className={`${base} bg-zinc-100 text-zinc-600 hover:bg-zinc-200 dark:bg-zinc-800 dark:text-zinc-400`}>{marker} ?</button>;
  };
    ...
    return (
      <div className="relative flex min-h-screen items-center justify-center bg-zinc-50 px-4 dark:bg-zinc-950">
        <div className="w-full max-w-md rounded-[2rem] border border-zinc-200/50 bg-white p-10 text-center shadow-md dark:border-zinc-800/50 dark:bg-zinc-900">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">LIMS Учёт ДНК</h1>
          <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Внутренняя система лаборатории</p>
          <button type="button" onClick={() => signIn()} className="mt-8 w-full rounded-full bg-teal-600 py-3.5 text-sm font-bold text-white shadow-md hover:bg-teal-700 hover:shadow-lg transition-all">
            Войти в систему
          </button>
        </div>
      </div>
  ...
  return (
    <div ref={ptrRef} className={MD3.page}>
      ...
      <header className={`mb-6 p-5 print:hidden relative z-10 ${MD3.card}`}>
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            <h1 className="flex flex-wrap items-center gap-3 text-xl font-bold tracking-tight text-zinc-900 sm:text-2xl dark:text-zinc-50">
              ...
              База Проб
              {isOnline && <Wifi className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />}
            </h1>
            <p className="mt-2 text-sm font-medium text-zinc-500 dark:text-zinc-400">Доступ: {role === 'ADMIN' ? 'Администратор' : role === 'EDITOR' ? 'Лаборант' : 'Чтение'}</p>
          </div>
          ...
          <div className="flex w-full min-w-0 flex-col gap-3 md:w-auto md:flex-1 md:items-end">
            ...
            <div className="hidden flex-wrap justify-end gap-2 md:flex">
              <button onClick={() => setShortcutsOpen(true)} className={MD3.iconBtn} title="Клавиши"><Keyboard className="h-4 w-4" /></button>
              <button onClick={() => setShowAdvFilter(!showAdvFilter)} className={`${MD3.iconBtn} ${showAdvFilter ? 'text-teal-600 bg-teal-100 dark:bg-teal-900/30' : ''}`} title="Умный фильтр"><Filter className="h-4 w-4" /></button>
              {!isReader && <button onClick={() => setIsAddModalOpen(true)} className={MD3.btnPrimary}><Plus className="h-4 w-4" />Новая проба</button>}
              <button onClick={exportToExcel} className={MD3.btnSecondary} title="Вся база"><Download className="h-4 w-4" />База</button>
              <button onClick={() => fetchSpecimens()} disabled={dataLoading} className={MD3.iconBtn}><RefreshCw className={`h-4 w-4 ${dataLoading ? 'animate-spin' : ''}`} /></button>
              <button onClick={() => setDarkMode(!darkMode)} className={MD3.iconBtn}>{darkMode ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button>
              {role === 'ADMIN' && <Link href="/admin" className={`${MD3.btnSecondary} !bg-violet-100 !text-violet-700 dark:!bg-violet-900/30 dark:!text-violet-300`}><Settings className="h-4 w-4" />Админ</Link>}
            </div>
            <div className="flex flex-wrap gap-2 md:hidden">
              {!isReader && <button onClick={() => setIsAddModalOpen(true)} className={`touch-target flex-1 ${MD3.btnPrimary}`}><Plus className="h-4 w-4" />Новая</button>}
              <button onClick={() => fetchSpecimens()} className={`touch-target ${MD3.iconBtn}`}><RefreshCw className={`h-5 w-5 ${dataLoading ? 'animate-spin' : ''}`} /></button>
              <button onClick={() => setToolsSheetOpen(true)} className={`touch-target flex-1 ${MD3.btnSecondary}`}><MoreHorizontal className="h-5 w-5" />Ещё</button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 border-t border-zinc-100 pt-5 sm:grid-cols-3 dark:border-zinc-800 mt-4">
          <div className={MD3.statCard}>
            <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500">Всего проб</p>
            <p className="mt-1 text-3xl font-bold">{specimens.length}</p>
          </div>
          <div className={MD3.statCard}>
            <p className="text-[11px] font-bold uppercase tracking-wider text-emerald-600">Успешные ITS</p>
            <p className="mt-1 text-3xl font-bold text-emerald-700 dark:text-emerald-400">{successCount}</p>
          </div>
          <div className={MD3.statCard}>
            <p className="text-[11px] font-bold uppercase tracking-wider text-rose-600">Остальные</p>
            <p className="mt-1 text-3xl font-bold text-rose-700 dark:text-rose-400">{specimens.length - successCount}</p>
          </div>
        </div>

        <div className="mt-4 h-2 overflow-hidden rounded-full bg-zinc-200/80 dark:bg-zinc-800/80">
          <div className="h-full rounded-full bg-teal-500 transition-[width] duration-500" style={{ width: `${successPercent}%` }} />
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {(['ALL', 'SUCCESS', 'ERROR', 'FAVORITES'] as const).map((key) => (
            ...
            >
              {key === 'FAVORITES' ? <span className="flex items-center gap-1.5"><Star className="h-4 w-4" />Избранное</span> : key === 'ALL' ? 'Все' : key === 'SUCCESS' ? 'Успешные' : 'С ошибками'}
            </button>
          ))}
        </div>
      </header>
      ...
      {!isReader && selectedIds.length > 0 && (
        <div className={`sticky top-3 z-20 mb-6 flex flex-col gap-4 p-5 shadow-lg print:hidden ${MD3.card} !bg-teal-50 dark:!bg-teal-900/20 !border-teal-200 dark:!border-teal-800/50`}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="font-bold text-teal-900 dark:text-teal-100">Выбрано: {selectedIds.length}</p>
            <div className="flex flex-wrap gap-2">
              <button onClick={exportExtractionJournal} className={MD3.btnSecondary}><FileSpreadsheet className="h-4 w-4" /> Журнал выделений</button>
              <button onClick={() => setSelectedIds(filteredSpecimens.map(s => s.id))} className={MD3.btnSecondary}>Все в фильтре</button>
              {isAdmin && <button onClick={handleMassDelete} className="rounded-full bg-rose-100 px-5 py-2.5 text-sm font-bold text-rose-700 hover:bg-rose-200 dark:bg-rose-900/30 dark:text-rose-400"><Trash2 className="h-4 w-4 inline mr-1" /> Удалить</button>}
            </div>
          </div>
      ...
      <div className={`overflow-hidden relative z-10 print:border-none print:shadow-none print:bg-transparent ${MD3.card}`}>
              ...
              {dataLoading ? (
                 Array.from({ length: 5 }).map((_, i) => (
                  <tr key={`sk-${i}`} className={MD3.tableRow}>
                    {!isReader && <td className="p-4"><div className="h-4 w-4 bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded" /></td>}
                    <td className="p-4"><div className="h-4 w-20 bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded-full" /></td>
                    <td className="p-4"><div className="h-4 w-32 bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded-full" /></td>
                    <td className="hidden xl:table-cell p-4"><div className="h-4 w-full bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded-full" /></td>
                    <td className="p-4"><div className="h-4 w-24 bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded-full" /></td>
                    <td className="p-4"><div className="h-6 w-20 bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded-full" /></td>
                    <td className="p-4"><div className="h-8 w-8 bg-zinc-200 dark:bg-zinc-700 animate-pulse rounded-full mx-auto" /></td>
                  </tr>
                 ))
              ) : currentData.length === 0 ? (
                <tr><td colSpan={tableColSpan} className="p-16 text-center text-zinc-500 font-medium">Ничего не найдено.</td></tr>
              ) : (
                currentData.map((s) => (
                  <tr key={s.id} className={`${MD3.tableRow} ${selectedIds.includes(s.id) ? 'bg-teal-50/50 dark:bg-teal-900/10' : ''}`}>
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap items-center gap-1.5 font-mono font-bold text-teal-700 dark:text-teal-400 text-base">
                        ...
                        <button type="button" onClick={() => copySpecimenLink(s.id)} className="text-zinc-400 hover:text-teal-600 print:hidden ml-1"><Link2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="font-bold">{s.taxon ? <HighlightMatch text={String(s.taxon)} query={search} /> : '—'}</div>
                      <div className="text-xs text-zinc-500 mt-1">{s.locality}</div>
                    </td>
                    <td className="hidden max-w-[14rem] align-top p-4 xl:table-cell">
                      {s.notes ? <p className="line-clamp-3 whitespace-pre-wrap break-words text-xs text-zinc-600 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-800/50 p-2 rounded-xl" title={s.notes}>{s.notes}</p> : <span className="text-xs text-zinc-400">—</span>}
                    </td>
                    <td className="p-4">
                      <div className="font-bold">{s.extrLab || '—'} <span className="font-normal text-zinc-500">{s.extrOperator ? `· ${s.extrOperator}` : ''}</span></div>
                      <div className="text-xs text-zinc-500 mt-1 bg-zinc-100 dark:bg-zinc-800 inline-block px-2 py-0.5 rounded-md">{s.extrMethod || 'Не указан'}</div>
                    </td>
                    ...
                    <td className="p-4 text-center print:hidden">
                      <div className="flex items-center justify-center gap-2">
                          {!isReader && (
                              <button type="button" onClick={() => setEditingSpecimen(s)} className="p-2 text-zinc-400 hover:text-teal-600 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded-full transition-colors" title="Изменить">
                                <Pencil className="h-4 w-4" />
                              </button>
                          )}
                          <button type="button" onClick={() => setPcrModalId(s.id)} className={`p-2 rounded-full transition-colors ${s.attempts?.length ? 'text-teal-600 bg-teal-50 dark:bg-teal-900/30' : 'text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`} title="ПЦР">
        ...
        {totalPages > 1 && (
          <div className="flex flex-wrap items-center justify-center gap-4 border-t border-zinc-100 bg-zinc-50/50 p-5 text-sm dark:border-zinc-800 dark:bg-zinc-900/30 print:hidden">
            <button disabled={currentPage <= 1} onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} className={MD3.btnSecondary}>Назад</button>
            <span className="font-bold text-zinc-600 dark:text-zinc-400">Стр. {currentPage} из {totalPages}</span>
            <button disabled={currentPage >= totalPages} onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} className={MD3.btnSecondary}>Вперёд</button>
          </div>
        )}
      </div>
      ...
      <nav className="safe-pb fixed bottom-0 left-0 right-0 z-[70] flex items-stretch justify-around gap-0 border-t border-zinc-200/90 bg-white/95 px-2 pt-2 pb-1 shadow-[0_-8px_30px_rgba(0,0,0,0.05)] backdrop-blur-md print:hidden md:hidden dark:border-zinc-800 dark:bg-zinc-900/95">
        <button type="button" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="touch-target flex flex-1 flex-col items-center justify-center py-1 text-[11px] font-medium text-zinc-600 hover:text-teal-600"><HomeIcon className="h-6 w-6 mb-1" />Вверх</button>
        <button type="button" onClick={() => searchInputRef.current?.focus()} className="touch-target flex flex-1 flex-col items-center justify-center py-1 text-[11px] font-medium text-zinc-600 hover:text-teal-600"><Search className="h-6 w-6 mb-1" />Поиск</button>
        <button type="button" onClick={() => setScanOpen(true)} className="touch-target flex flex-1 flex-col items-center justify-center py-1 text-[11px] font-bold text-teal-700"><ScanLine className="h-6 w-6 mb-1" />Скан</button>
        {!isReader && <button type="button" onClick={() => setIsAddModalOpen(true)} className="touch-target flex flex-1 flex-col items-center justify-center py-1 text-[11px] font-bold text-teal-700"><Plus className="h-6 w-6 mb-1" />Проба</button>}
        <button type="button" onClick={() => setToolsSheetOpen(true)} className="touch-target flex flex-1 flex-col items-center justify-center py-1 text-[11px] font-medium text-zinc-600 hover:text-teal-600"><MoreHorizontal className="h-6 w-6 mb-1" />Меню</button>
      </nav>

      {toolsSheetOpen && (
        <div className="fixed inset-0 z-[110] md:hidden print:hidden">
          <button type="button" className="absolute inset-0 bg-zinc-950/40" onClick={() => setToolsSheetOpen(false)} />
          <div className="safe-pb absolute bottom-0 left-0 right-0 max-h-[88dvh] overflow-y-auto rounded-t-[2rem] bg-white px-5 pb-5 pt-3 shadow-2xl dark:bg-zinc-900 transition-transform animate-in slide-in-from-bottom">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <button onClick={() => { setToolsSheetOpen(false); exportExtractionJournal(); }} className="flex min-h-[56px] items-center justify-center gap-3 rounded-2xl bg-teal-50 text-sm font-bold text-teal-900 dark:bg-teal-900/30 dark:text-teal-100"><FileSpreadsheet className="h-5 w-5" /> Журнал выделений</button>
              <button onClick={() => { setToolsSheetOpen(false); exportToExcel(); }} className="flex min-h-[56px] items-center justify-center gap-3 rounded-2xl bg-zinc-100 text-sm font-bold text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100"><Download className="h-5 w-5" /> Вся база (Excel)</button>
              <button onClick={() => { setToolsSheetOpen(false); exportCsv(); }} className="flex min-h-[56px] items-center justify-center gap-3 rounded-2xl bg-zinc-100 text-sm font-bold text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100"><FileText className="h-5 w-5" /> Список (CSV)</button>
              <button onClick={() => { setToolsSheetOpen(false); window.print(); }} className="flex min-h-[56px] items-center justify-center gap-3 rounded-2xl bg-zinc-100 text-sm font-bold text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100"><Printer className="h-5 w-5" /> Печать</button>
              {role === 'ADMIN' && <Link href="/admin" onClick={() => setToolsSheetOpen(false)} className="flex min-h-[56px] items-center justify-center gap-3 rounded-2xl bg-violet-100 text-sm font-bold text-violet-900 dark:bg-violet-900/30 dark:text-violet-100"><Settings className="h-5 w-5" /> Панель Админа</Link>}
            </div>
            <button onClick={() => { setToolsSheetOpen(false); signOut(); }} className="mt-5 flex w-full min-h-[56px] items-center justify-center gap-3 rounded-full bg-zinc-900 text-sm font-bold text-white dark:bg-zinc-100 dark:text-zinc-900"><LogOut className="h-5 w-5" /> Выйти</button>
          </div>
        </div>
      )}
