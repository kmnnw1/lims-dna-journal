...
import { requireRole, handleError, invalidateSpecimenCaches, type ApiUser } from '../specimens/helpers';
...
export async function GET(req: Request) {
    try {
        ...
        if (resourceType === 'SPECIMEN') {
            whereClause = {
                OR: [
                    ...
                    { resourceType: 'PCR_ATTEMPT', details: { contains: `"specimenId":"${resourceId}"` } },
                    ...
                    { resourceType: 'PCR_ATTEMPT', details: { contains: `"specimenId":"${resourceId}"` }, action: { in: ['CREATE_PCR_ATTEMPT', 'DELETE_SPECIMEN'] } }
                ]
            };
        }
        ...
        return NextResponse.json(history.map((item: any) => ({
            ...item,
...
export async function POST(req: Request) {
    try {
        ...
        if (!logEntry || !logEntry.changes || !logEntry.resourceId) {
            return NextResponse.json({ error: 'Запись лога не найдена или не содержит данных для отката' }, { status: 404 });
        }
