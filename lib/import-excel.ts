...
/** Достаём первую дату вида дд.мм.гг из длинной ячейки (например «PCR, FaM, 21.11.14»). */
...
/**
 * Ключ колонки по заголовку. Порядок важен: сначала длинные шаблоны (its gb, не its).
 */
...
export function parseLabDate(raw: string): Date | null {
  ...
  const romanRe =
    /(\d{1,2})\s+(xii|xi|x|ix|viii|vii|vi|iv|iii|ii|i|v)\s+(\d{2,4})/i;
...
export function parseRowWithBindings(
  ...
  noteParts.push(`[импорт] лист «${sheetName}», строка ${sheetRowIndex0 + 1}`);
...
export function parseRowLegacy(
  ...
  const meta = `[лист «${sheetName}»; строка ${sheetRowIndex0 + 1}]`;
  const fromCells = collectRowCellComments(sheet, sheetRowIndex0);
  const notes = [meta, unmapped.join(" | "), fromCells].filter(Boolean).join("\n").trim();
...
/**
 * Лист LY: без строки «Isolate», колонки A–H — ID, таксон, коммент/дата, место, коллектор, гербарий, лаборатория, метод.
 */
export function parseLySheetRows(
  ...
  for (let i = 0; i < rawData.length; i++) {
    ...
    if (!id || /^isolate$/i.test(id)) continue;
    ...
    const noteParts = [
      `[импорт] лист «${sheetName}», строка ${i + 1}`,
...
export function mergeById(rows: ParsedSpecimenRow[]): ParsedSpecimenRow[] {
  ...
  for (const row of rows) {
    ...
    const mergedNotes = [prev.notes, row.notes].filter(Boolean).join("\n---\n");
    ...
    const extrDateComputed =
      extrDateString(pDate) ||
      pickNonEmpty(prev.extrDate ?? "", row.extrDate ?? "").trim() ||
      null;
