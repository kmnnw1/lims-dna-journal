      ...
      <div className={`${MD3.card} my-4 w-full max-w-xl p-8 relative`}>
            ...
            activeSpecimen.attempts.map((a: any) => (
              <div key={a.id} className="rounded-[1.5rem] bg-zinc-50 p-5 dark:bg-zinc-800/50">
                <div className="flex flex-wrap justify-between items-center gap-3 mb-3">
                  ...
                  <span className={`font-bold px-3 py-1 rounded-full text-xs uppercase tracking-wider ${a.result === 'Success' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-400'}`}>
                    {pcrResultLabelRu(a.result)}
                  </span>
                </div>
                {(a.marker || a.forwardPrimer || a.reversePrimer || a.dnaMatrix) && (
                  <div className="flex flex-wrap gap-2 text-xs font-medium">
                    {a.marker && <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 shadow-sm border border-zinc-100 dark:border-zinc-800">Маркер: <b>{a.marker}</b></span>}
                    {a.forwardPrimer && <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 shadow-sm border border-zinc-100 dark:border-zinc-800">F: {a.forwardPrimer}</span>}
                    {a.reversePrimer && <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 shadow-sm border border-zinc-100 dark:border-zinc-800">R: {a.reversePrimer}</span>}
                    {a.dnaMatrix && <span className="px-3 py-1.5 rounded-lg bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 shadow-sm border border-zinc-100 dark:border-zinc-800">Матрица: {a.dnaMatrix}</span>}
                  </div>
                )}
              </div>
            ))
        ...
        {!isReader && (
          <div className="flex flex-col gap-4 pt-6 border-t-2 border-zinc-100 dark:border-zinc-800">
            <p className="text-[11px] font-bold uppercase tracking-wider text-teal-600 mb-2">Новая попытка</p>
            <div className="grid grid-cols-2 gap-4">
              <input placeholder="Объём (мкл)" value={pcrForm.volume} onChange={(e) => setPcrForm({ ...pcrForm, volume: e.target.value })} className={MD3.input} />
              <input placeholder="Маркер (ITS...)" value={pcrForm.marker} onChange={(e) => setPcrForm({ ...pcrForm, marker: e.target.value })} className={MD3.input} />
              <input placeholder="Fwd праймер" value={pcrForm.forwardPrimer} onChange={(e) => setPcrForm({ ...pcrForm, forwardPrimer: e.target.value })} className={MD3.input} />
              <input placeholder="Rev праймер" value={pcrForm.reversePrimer} onChange={(e) => setPcrForm({ ...pcrForm, reversePrimer: e.target.value })} className={MD3.input} />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <input placeholder="Матрица ДНК (конц. / объём)" value={pcrForm.dnaMatrix} onChange={(e) => setPcrForm({ ...pcrForm, dnaMatrix: e.target.value })} className={MD3.input} />
              <select value={pcrForm.result} onChange={(e) => setPcrForm({ ...pcrForm, result: e.target.value })} className={`${MD3.input} cursor-pointer`}>
