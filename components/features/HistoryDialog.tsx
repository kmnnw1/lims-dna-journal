...
export function HistoryDialog({ resourceId, resourceType, open, onClose, onRestoreSuccess }: HistoryDialogProps) {
    ...
    const fetchHistory = async () => {
        ...
        setError(null);
        try {
            const res = await fetch(`/api/audit?resourceType=${resourceType}&resourceId=${resourceId}`);
                    ...
                    <div className="flex-1 overflow-y-auto p-6 scrollbar-thin">
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center py-12 gap-4">
                                ...
                                <p className="text-[var(--md-sys-color-on-surface-variant)] font-medium">Загрузка истории...</p>
                            </div>
                        ) : error ? (
                            <div className="flex flex-col items-center justify-center py-12 text-center">
                                <AlertCircle className="w-12 h-12 text-[var(--md-sys-color-error)] mb-3" />
                                <p className="text-lg font-bold text-[var(--md-sys-color-on-surface)]">{error}</p>
                                <button
                            ...
                            <div className="flex flex-col items-center justify-center py-12 text-center opacity-60">
                                ...
                                <p className="text-lg font-medium">Логов для этой пробы не найдено</p>
                            </div>
                        ) : (
                                                ...
                                                <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
                                                    ...
                                                    <div className="flex items-center gap-3 text-[11px] font-bold text-[var(--md-sys-color-on-surface-variant)]/70">
                                                        <div className="flex items-center gap-1">
                                                            ...
                                                            {log.userId === 'admin' ? 'Системный администратор' : log.userId}
                                                        </div>
                                                        <div className="flex items-center gap-1">
                                                ...
                                                {changes && Object.keys(changes).length > 0 ? (
                                                    <div className="grid gap-2">
                                                        {Object.entries(changes).map(([key, value]: [string, any]) => (
                                                            <div key={key} className="flex flex-col gap-1 p-2.5 rounded-xl bg-black/5 dark:bg-white/5 border border-[var(--md-sys-color-outline-variant)]/10">
