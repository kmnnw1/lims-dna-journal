    ...
    return (
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-8">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                ...
                <button onClick={() => setIsDark(!isDark)} title="Тема" className="p-3.5 bg-[var(--md-sys-color-surface-container)] hover:bg-[var(--md-sys-color-surface-container-high)] rounded-full transition-all flex items-center justify-center">
                    {isDark ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
                </button>

                <button onClick={onAddClick} className="flex items-center gap-2 px-5 py-3.5 bg-[var(--md-sys-color-primary-container)] text-[var(--md-sys-color-on-primary-container)] hover:shadow-md rounded-[1rem] transition-all font-medium active:scale-95">
                    <Plus className="w-6 h-6" />
                    <span className="hidden sm:inline">Новая проба</span>
                </button>

                <Link href="/admin" className="flex items-center gap-2 px-4 py-3.5 bg-[var(--md-sys-color-surface-container)] hover:bg-[var(--md-sys-color-surface-container-high)] rounded-[1rem] transition-all font-medium">
                    <Settings className="w-5 h-5" />
                    <span className="hidden sm:inline">Админ</span>
                </Link>

                <button onClick={onSignOut} title="Выйти" className="p-3.5 bg-[var(--md-sys-color-surface-container)] hover:bg-[var(--md-sys-color-error-container)] hover:text-[var(--md-sys-color-on-error-container)] rounded-full transition-all flex items-center justify-center">
