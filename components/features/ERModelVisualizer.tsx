...
import { Database, Fingerprint, FlaskConical, Dna, UserSquare2, ShieldCheck, FileSpreadsheet, Lock } from 'lucide-react';
...
const NODES: DBNode[] = [
    {
        id: 'specimen', label: 'Specimen', desc: 'Пробы и образцы', icon: Fingerprint,
        baseX: 20, baseY: 50, phaseX: 0.1, phaseY: 0.3, color: 'var(--md-sys-color-primary)', bg: 'var(--md-sys-color-primary-container)',
        fullDesc: [
            { key: 'PK', name: 'id (UUID)' },
    ...
    {
        id: 'extraction', label: 'DNA Extraction', desc: 'Выделение ДНК', icon: FlaskConical,
        baseX: 50, baseY: 20, phaseX: 0.5, phaseY: 0.8, color: 'var(--md-sys-color-secondary)', bg: 'var(--md-sys-color-secondary-container)',
        fullDesc: [
            { key: 'PK', name: 'id (UUID)' },
    ...
    {
        id: 'pcr', label: 'PCR Reaction', desc: 'ПЦР скрининг', icon: Dna,
        baseX: 80, baseY: 50, phaseX: 0.8, phaseY: 0.1, color: 'var(--md-sys-color-tertiary)', bg: 'var(--md-sys-color-tertiary-container)',
        fullDesc: [
            { key: 'PK', name: 'id (UUID)' },
    ...
    {
        id: 'users', label: 'System Users', desc: 'Пользователи LIMS', icon: UserSquare2,
        baseX: 50, baseY: 80, phaseX: 0.2, phaseY: 0.9, color: 'var(--md-sys-color-error)', bg: 'var(--md-sys-color-error-container)',
        fullDesc: [
            { key: 'PK', name: 'id (UUID)' },
...
export function ERModelVisualizer() {
        ...
        const tick = () => {
            ...
            if (dims.width > 0) {
                NODES.forEach(n1 => {
                    ...
                    // Wall bouncing logic
                    if (nodePhys.x < 120) { nodePhys.x = 120; nodePhys.vx *= -0.5; }
                    if (nodePhys.x > dims.width - 120) { nodePhys.x = dims.width - 120; nodePhys.vx *= -0.5; }
                    if (nodePhys.y < 80) { nodePhys.y = 80; nodePhys.vy *= -0.5; }
                    if (nodePhys.y > dims.height - 80) { nodePhys.y = dims.height - 80; nodePhys.vy *= -0.5; }
                });
            }
    ...
    return (
        ...
        >
            <div className="absolute inset-0 opacity-5" style={{ backgroundImage: 'radial-gradient(var(--md-sys-color-on-surface) 1.5px, transparent 1.5px)', backgroundSize: '30px 30px' }} />
            ...
            <svg className="absolute inset-0 w-full h-full pointer-events-none">
                <defs>
                    {LINKS.map((link, i) => {
                        ...
                        return (
                            <linearGradient key={`grad-${i}`} id={`glowLine-${i}`} x1="0%" y1="0%" x2="100%" y2="100%">
                ...
                {LINKS.map((link, i) => {
                    ...
                    return (
                        <g key={i}>
                            ...
                            />
                            {bothExpanded && (
                                <g transform={`translate(${midX}, ${midY}) rotate(${angle}) translate(0, -10)`}>
                                    <rect x="-70" y="-12" width="140" height="24" fill="var(--md-sys-color-surface-container-highest)" rx="12" className="shadow-sm" />
                                    <text x="0" y="4" textAnchor="middle" fontSize="11" fill={`url(#glowLine-${i})`} fontWeight="700" style={{ letterSpacing: '0.5px' }}>
                ...
                return (
                    <div 
                        ...
                        style={{ 
                            ...
                            transition: isDragging ? 'none' : 'box-shadow 0.3s, transform 0.2s, background-color 0.3s'
                        }}
                    >
                        <div className="flex items-center gap-3 w-full pointer-events-none">
                            <Icon className="w-8 h-8 opacity-80 shrink-0" />
                            <div className="flex-1 min-w-0">
                                <div className="font-medium tracking-tight text-sm uppercase opacity-90 truncate">{n.label}</div>
                                {!expanded && <div className="text-xs opacity-70 font-mono mt-0.5 truncate">{n.desc}</div>}
                            </div>
                        </div>
                        
                        {expanded && (
                            <div className="mt-4 pt-3 border-t border-[var(--md-sys-color-outline)]/20 space-y-2 pointer-events-none">
                                <div className="text-xs font-medium opacity-80 uppercase tracking-widest text-[var(--md-sys-color-on-surface)]">{n.desc}</div>
                                <div className="space-y-1 mt-2">
                                    {n.fullDesc.map((attr, idx) => (
                                        <div key={idx} className="flex items-center justify-between text-xs font-mono">
                                            <span className={`px-1.5 py-0.5 rounded ${attr.key === 'PK' ? 'bg-[var(--md-sys-color-primary)] text-white' : attr.key === 'FK' ? 'bg-[var(--md-sys-color-tertiary)] text-white' : 'bg-black/5 dark:bg-white/5 opacity-70'}`}>
                                                {attr.key}
                                            </span>
            ...
            <div className="absolute top-6 left-8 flex flex-col gap-1 text-[var(--md-sys-color-on-surface)] opacity-40 pointer-events-none">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium uppercase tracking-widest">DNA Lab ER-Model</span>
                </div>
