...
export function MobileSpecimenCard({
  ...
  let articleClass =
    "transition-all duration-300 relative rounded-[2rem] p-5 border shadow-sm ";
  if (selected) {
    articleClass +=
      "border-teal-400 bg-teal-50 dark:border-teal-700/50 dark:bg-teal-900/20";
  } else {
    articleClass +=
    ...
    <article className={articleClass}>
      ...
      <div className="mt-4 pt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-zinc-200/50 dark:border-zinc-800/50 text-sm">
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-xs text-zinc-500">{s.extrMethod || <span className="text-zinc-300">Нет данных</span>}</p>
        </div>
