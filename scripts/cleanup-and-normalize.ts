...
async function cleanup() {
    ...
    const davydovAliases = ['давыдов 5315', 'f. a. davydov 5295', 'e. a. davydov 5309', 'e. a. davydov', 'давыдов', 'davydov'];
    ...
    for (const s of allSpecimens) {
        ...
        for (const name of names) {
            if (davydovAliases.some(a => name.toLowerCase().includes(a)) || name.toLowerCase().includes('давыдов')) {
                uniqueNames.add(davydovName);
            } else {
